...
function files(filename, flag) {
	...
	if (stringContains(flag, "w")) {
		...
		this.write = function(data) {
			try {
				FileUtils.writeStringToFile(new File(Globals.getCWD()+filename), data, append);
			} catch (e) {
...
files.move = function(sourceFileName, destFileName) {
	...
	if (sourcefile.exists()) {
		...
		if (sourcefile.isDirectory()) {
			...
			//If the source is a file, or unknown type, attempt to move it as a file
		} else {
...
/** Make a directory.
 * @param dirname The string name of the directory
 * @param recursive Whether or not to create multiple nested directories as indicated by dirname
 */
...
files.find = function(dirname, recursive, extensions) {
	...
	return FileUtils.listFiles(new File(Globals.getCWD()+dirname), extensions, recursive);
}
...
/* A string representing the location of the user's home directory. MAY MISBEHAVE ON WINDOWS VISTA+ */
